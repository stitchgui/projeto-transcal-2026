"""Regime transitório da esfera vitrocerâmica.
...
radiativa T_surr e o coeficiente combinado convecção+radiação
...
Atenção: o Biot do critério lumped x distribuído
usa L_c,esf=D/6; o Biot que entra na equação de autovalores da série
...
from transcal.balance.solver import rayleigh_number

# --------------------------------------------------------------------------- #
# Cinemática e critério de regime convectivo
# --------------------------------------------------------------------------- #


def sphere_velocity(W, t_total_s):
    """Velocidade média de travessia da esfera (SPEC §6.1)."""
    return W / t_total_s


def grashof_over_reynolds2(Ts_K, T_int_K, D, V_esf, g):
...
# --------------------------------------------------------------------------- #
# Coeficientes convectivo e radiativo da esfera
# --------------------------------------------------------------------------- #


def nusselt_sphere(Ra_D, Pr):
    """Eq. 7.2 -- Churchill (1975), válida para Ra_D<=1e11, Pr>=0.7."""
    if Ra_D > 1e11 or Pr < 0.7:
        print(f"AVISO: Correlação de Churchill (esfera) fora do domínio de validade: Ra_D={Ra_D:.3e}, Pr={Pr:.3f}")
    return 2.0 + 0.589 * Ra_D**0.25 / (1.0 + (0.469 / Pr) ** (9 / 16)) ** (4 / 9)


def h_rad_sphere(Ts_K, T_surr_K, eps_esf, sigma):
...
def surrounding_radiation_temperature(T_K, areas, eps):
...
# --------------------------------------------------------------------------- #
# Série analítica de autovalores
# --------------------------------------------------------------------------- #


def sphere_eigenvalues(Bi, n_terms):
    ...
    for n in range(1, n_terms + 1):
        lo = (n - 1) * np.pi + 1e-9
        hi = n * np.pi - 1e-9
        roots[n - 1] = optimize.brentq(f, lo, hi)  # type: ignore
    return roots


def series_coefficients(zeta):
...
def _effective_n_terms(zeta, Fo_min, tol):
...
def eigenseries(Bi, Fo_min, max_terms=400, tol=1e-12):
...
# --------------------------------------------------------------------------- #
# Parâmetros da esfera e orquestração
# --------------------------------------------------------------------------- #


class SphereParams:
    def __init__(self, D, k, rho, cp, eps, T0_C, W_chamber, t_total_s):
        self.D = D                  # m
        self.k = k                   # W/m.°C
        self.rho = rho                # kg/m3
        self.cp = cp                    # J/kg.°C
        self.eps = eps                    # emissividade
        self.T0_C = T0_C                    # °C, condição inicial
        self.W_chamber = W_chamber            # m, comprimento percorrido (eixo x da câmara)
        self.t_total_s = t_total_s              # s, tempo total de travessia (120 min)


def default_sphere_params():
...
class SphereTransientResult:
    def __init__(
        self,
        t_min,            # (n,) minutos, 0..120
        T_core_C,           # (n,) °C
        T_surface_C,          # (n,) °C
        Bi_series,              # h_total*r0/k -- Biot do autovalor (Incropera, r0=D/2)
        Bi_lumped,                # h_total*(D/6)/k -- critério lumped x distribuído
        h_conv,                     # W/m².K
        h_rad,                        # W/m².K
        h_total,                       # W/m².K
        Ra_D,
        T_eff_C,                         # °C, acionamento combinado conv+rad
        T_surr_C,                         # °C, Eq. 7.4
        T_int_C,                            # °C, importado de balance/solver.py
        Gr_over_Re2,                          # Eq. 7.1
        n_terms,
        n_iter,                                 # iterações de autoconsistência de T_s,rep
    ):
        self.t_min = t_min
        self.T_core_C = T_core_C
        self.T_surface_C = T_surface_C
        self.Bi_series = Bi_series
        self.Bi_lumped = Bi_lumped
        self.h_conv = h_conv
        self.h_rad = h_rad
        self.h_total = h_total
        self.Ra_D = Ra_D
        self.T_eff_C = T_eff_C
        self.T_surr_C = T_surr_C
        self.T_int_C = T_int_C
        self.Gr_over_Re2 = Gr_over_Re2
...
def compute_transient_profile(
    ...
    for n_iter in range(1, max_iter + 1):
        ...
        print(
            f"autoconsistência it={n_iter:<2d} T_s,rep={T_s_rep_K - 273.15:6.2f}°C -> "
            f"{T_s_rep_new_K - 273.15:6.2f}°C  Bi_series={Bi_series:.4f}  h_total={h_total:.3f} W/m².K"
        )
...
# --------------------------------------------------------------------------- #
# Plot e exportação automática em disco
# --------------------------------------------------------------------------- #


def plot_sphere_history(result):
    ...
    T0_C = result.T_core_C[0]
    ax.axhline(T0_C, color="gray", linestyle="--", linewidth=1, label=f"T₀ = {T0_C:.0f} °C (condição inicial)")
    ax.set_xlabel("Tempo [min]")
    ax.set_ylabel("Temperatura [°C]")
    ax.set_title("Histórico térmico da esfera vitrocerâmica (0-120 min)")
    ax.set_xlim(0, result.t_min[-1])
    ax.set_ylim(T0_C - 5, max(result.T_surface_C.max(), result.T_core_C.max()) + 10)
    ax.grid(True, alpha=0.3)
    ax.legend()
    fig.tight_layout()

    os.makedirs("figures", exist_ok=True)
    out_path = "figures/sphere_temperature_history.png"
